...
def create_file_logger(logger_name, file_path, level=logging.DEBUG):
    """
    创建一个logger，用于将日志信息输出到指定的文件，并在每次创建时清空原有日志。
    ...
    # 检查是否已经存在handler，如果存在则移除，避免重复日志
    if logger.hasHandlers():
        logger.handlers.clear()

    # 创建一个handler，用于写入日志文件，模式设置为'w'以覆盖旧文件
...
def gpu2cpu_momentum_params_torch_vdb(params_idx, up_gpu_params, down_gpu_params, index):
    if "adamw" not in index.config.optimizer_kv:
        return
    if not hasattr(trainer_pointer, "optimizer"):
        optimizer = trainer_pointer.optimizers().optimizer
    elif trainer_pointer.optimizer is None:
        raise ValueError("trainer_pointer.optimizer is None")
    else:
        optimizer = trainer_pointer.optimizer
    # logger.info(f"{params_idx.shape}, {torch.max(params_idx)}")
    # logger.info(f"{trainer_pointer.optimizer.state[up_gpu_params].keys()}")
    # logger.info(f"{trainer_pointer.optimizer.state[up_gpu_params]['exp_avg'].shape}")
    logger.info(f"optimizer state dtype in gpu2cpu {optimizer.state[up_gpu_params]['exp_avg'].dtype}")

    index.linear_up_m1[params_idx, ...] = \
        optimizer.state[up_gpu_params]['exp_avg']. \
            to(device=index.device, dtype=index.linear_up.dtype, non_blocking=True)
    index.linear_up_m2[params_idx, ...] = \
        optimizer.state[up_gpu_params]['exp_avg_sq']. \
            to(device=index.device, dtype=index.linear_up.dtype, non_blocking=True)
    index.linear_up_step[params_idx] += 1

    index.linear_down_m1[..., params_idx] = \
        optimizer.state[down_gpu_params]['exp_avg']. \
            to(device=index.device, dtype=index.linear_down.dtype, non_blocking=True)
    index.linear_down_m2[..., params_idx] = \
        optimizer.state[down_gpu_params]['exp_avg_sq']. \
            to(device=index.device, dtype=index.linear_down.dtype, non_blocking=True)
    index.linear_down_step[params_idx] += 1


def cpu2gpu_momentum_params_torch_vdb(params_idx, up_gpu_params, down_gpu_params, index):
    if "adamw" not in index.config.optimizer_kv:
        return
    if not hasattr(trainer_pointer, "optimizer"):
        optimizer = trainer_pointer.optimizers().optimizer
    ...
    #  todo 不该指定device，但是没办法
    device = torch.device("cuda")
    if 'exp_avg' not in optimizer.state[up_gpu_params]:
        return

    logger.info(
        f"optimizer state dtype in cpu2gpu {optimizer.state[up_gpu_params]['exp_avg'].dtype}")  # ?

    optimizer.state[up_gpu_params]['exp_avg'] = \
        index.linear_up_m1[params_idx, ...].to(device=device, dtype=up_gpu_params.dtype)
    optimizer.state[up_gpu_params]['exp_avg_sq'] = \
        index.linear_up_m2[params_idx, ...].to(device=device, dtype=up_gpu_params.dtype)
    optimizer.state[up_gpu_params]['step'] = \
        index.linear_up_step[params_idx]

    optimizer.state[down_gpu_params]['exp_avg'] = \
        index.linear_down_m1[..., params_idx].to(device=device, dtype=down_gpu_params.dtype)
    optimizer.state[down_gpu_params]['exp_avg_sq'] = \
        index.linear_down_m2[..., params_idx].to(device=device, dtype=down_gpu_params.dtype)
    optimizer.state[down_gpu_params]['step'] = \
        index.linear_down_step[params_idx]

    # assert optimizer.state[up_gpu_params]['step'] != 0, "step == 0"
    # assert optimizer.state[down_gpu_params]['step'] != 0, "step == 0"
    if not torch.isfinite(optimizer.state[up_gpu_params]['exp_avg']).all():
        logger.info(f"\n\n{str(optimizer.state[up_gpu_params]['exp_avg'])}")
    assert torch.isfinite(optimizer.state[up_gpu_params]['exp_avg']).all(), \
        "Invalid value detected"
    if not torch.isfinite(optimizer.state[up_gpu_params]['exp_avg_sq']).all():
        logger.info(f"\n\n{str(optimizer.state[up_gpu_params]['exp_avg_sq'])}")
    assert torch.isfinite(optimizer.state[up_gpu_params]['exp_avg_sq']).all(), \
        "Invalid value detected"
    assert torch.isfinite(optimizer.state[down_gpu_params]['exp_avg']).all(), \
        "Invalid value detected"
    assert torch.isfinite(optimizer.state[down_gpu_params]['exp_avg_sq']).all(), \
        "Invalid value detected"


def ffn_transfer(post_mlp, x):
    if post_mlp.topk_idx is not None:
        if 'adam' not in post_mlp.config.optimizer_kv:
            post_mlp.index.update(
                None if post_mlp.config.frozen_key
                else post_mlp.add_up_proj_gpu.data,
                post_mlp.add_down_proj_gpu.data,
                post_mlp.topk_idx
            )
        # gpu2cpu_momentum_params_torch_vdb(
        #     post_mlp.topk_idx,
        #     post_mlp.add_up_proj_gpu.weight,
        #     post_mlp.add_down_proj_gpu.weight,
        #     post_mlp.index
        # )
        post_mlp.topk_idx = None
        logger.info(f"ffn_transfer layer_idx={post_mlp.layer_idx}, gpu2cpu")

    up_w, down_w, idx = post_mlp.index.query_batch_direct(x, post_mlp.config.gpu_topk, post_mlp.layer_idx)
    if post_mlp.topk_idx is not None:
        raise ValueError("是异步的？")
    post_mlp.topk_idx = idx

    logger.info(f"up_w, up_gpu shape {up_w.shape}, {post_mlp.add_up_proj_gpu.data.shape}")
    logger.info(f"down_w down_gpu shape {down_w.shape}, {post_mlp.add_down_proj_gpu.data.shape}")

    # assert up_w.shape[1] == post_mlp.add_up_proj_gpu.data.shape[1]
    # assert down_w.shape[0] == post_mlp.add_down_proj_gpu.data.shape[0]
    # logger.info(f"down_w.grad_fn {down_w.grad_fn}")

    n = idx.shape[0]
    post_mlp.add_up_proj_gpu.data[:n, :] = up_w.to(
        dtype=post_mlp.add_up_proj_gpu.data.dtype,
        device=post_mlp.device,
        non_blocking=True
    ).data
    post_mlp.add_up_proj_gpu.data[n:, :] = 0.0
    post_mlp.add_down_proj_gpu.data[:, :n] = down_w.to(
        dtype=post_mlp.add_down_proj_gpu.data.dtype,
        device=post_mlp.device,
        non_blocking=True
    ).data
    post_mlp.add_down_proj_gpu[:, n:] = 0.0

    logger.info(f"after set {post_mlp.add_up_proj_gpu.shape} {post_mlp.add_down_proj_gpu.shape}")
    # logger.info(f"weight grad attr is {post_mlp.add_down_proj_gpu.requires_grad}")

    # cpu2gpu_momentum_params_torch_vdb(
    #     post_mlp.topk_idx,
    #     post_mlp.add_up_proj_gpu.weight,
    #     post_mlp.add_down_proj_gpu.weight,
    #     post_mlp.index
    # )
    logger.info(f"ffn_transfer layer_idx={post_mlp.layer_idx}, cpu2gpu")
    logger.info('-' * 40 + "ffn_transfer end!")


class KVLlamaConfig(LlamaConfig):
        ...
        if getattr(self.config, "moe_style", False):
            ...
            if lim := getattr(self.config, "limit_total_activated_neurons", 0):
                ...
                keys_weight = torch.zeros(bs * seq_len, self.config.on_gpu_size,
                                          device=self.add_down_proj_gpu.weight.device, dtype=_.dtype).scatter_(-1, ids, _)
            ...
            if self.config.gpu_topk <= 0:
                if not self.only_post:
                    # raise ValueError
                    if os.environ.get("DEBUG_KV", False):
                        o = self.relu(self.add_up_proj_gpu(x))
                        down_proj += self.add_down_proj_gpu(o)

                        bool_tensor = o.gt(1e-5)
                        count = bool_tensor.sum() / bool_tensor.numel()
                        self.unique_cnt.append(float(count))
                        base = 500 if os.environ.get("TRAINING", False) else 1
                        if len(self.unique_cnt) % (16 * base) == 0:  # write sometime
                            with open(f"/root/autodl-tmp/{getattr(self.config, 'run_name', '')}"
                                      f"-topk{self.config.gpu_topk}-{self.layer_idx}.pk", "wb") as pk:
                                pickle.dump(self.unique_cnt, pk)
...
class KVLlamaDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config, only_post=False, layer_idx=None):
        ...
        del self.mlp
        self.config = config
        self.mlp = KVLlamaMLP(config, only_post=only_post)
    ...
    def __init__(self, config):
        ...
        for i in range(config.num_hidden_layers):
            if i in kv_layers_id:
                self.layers.append(KVLlamaDecoderLayer(self.config, layer_idx=i))
            elif i in only_post_layers:
                self.layers.append(KVLlamaDecoderLayer(self.config, only_post=True, layer_idx=i))
            else:
                self.layers.append(LlamaDecoderLayer(self.config, layer_idx=i))
...
class KVLlamaForCausalLM(LlamaForCausalLM):
    def __init__(self, config: KVLlamaConfig):
        ...
        self.pretraining_tp = 0  # todo 由于trainer不知道哪里认为我将模型加载在了多个gpu上，因此必须设置这个参数
